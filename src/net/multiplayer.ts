// The client half of the relay in server/index.ts. Connects, pushes this browser's own
// corner of the world at connection time (spec: "envoyé au serveur à la connexion"), then
// keeps flushing whatever the local GameEngine marks dirty (own edits, or the side effects of
// visiting someone else's house — see GameEngine's "Multiplayer sync" section) and applies
// whatever the server relays back from everyone else. If the server is unreachable, the app
// just keeps working as a single-player world — nothing here is required for the base game.
...
export function connectMultiplayer(engine: GameEngine, identity: LocalIdentity): MultiplayerHandle {
  ...
  function snapshot(): ConnectionState {
    return { connected, onlinePlayerIds: new Set(onlinePlayerIds) };
  }
  ...
  function connect(): void {
    ...
    socket.onopen = () => {
      ...
      const hello: ClientMessage = { type: "hello", playerId: identity.id, name: identity.name };
